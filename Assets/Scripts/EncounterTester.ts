 ...
 * (per-frame `Base64.decode(...)` -> `addAudioFrame(pcmBytes, 1)`).
 */
...
import { EncounterMessage } from "../estuary-lens-studio-sdk/src/Models/EncounterMessage";
import { EncounterVoice } from "../estuary-lens-studio-sdk/src/Models/EncounterVoice";
import { EncounterEnd } from "../estuary-lens-studio-sdk/src/Models/EncounterEnd";

/**
 * Local duck-typed interface for `DynamicAudioOutput` from
 * `RemoteServiceGateway.lspkg`. Lifted from
 * `Assets/estuary-lens-studio-sdk/Examples/EstuaryVoiceConnection.ts:48-50`
 * so we don't depend on the package's type surface (it's not exported).
 */
...
export class EncounterTester extends BaseScriptComponent {
    ...
    onAwake(): void {
        ...
        // Frame-loop poll until EstuaryManager.instance is connected.
        // Mirrors the pattern in ConnectionIndicator.ts: createEvent("UpdateEvent")
        // -> bind a callback that checks connectionState every frame.
    ...
    private tryStartIfReady(): void {
        ...
        // Resolve the DynamicAudioOutput components if voice playback is on.
        if (this.voice) {
        ...
        // Register listeners BEFORE issuing startEncounter — the server
        // begins streaming as soon as the background task spins up, and
        // we don't want to miss the first emit if it races our subscribe.
    ...
    private onVoice(v: EncounterVoice): void {
        ...
        try {
            // Decode + frame-feed pattern lifted from
            // `Examples/EstuaryVoiceConnection.ts:404-415`. Lens Studio
            // exposes `Base64` as a global — decode returns a Uint8Array
            // of PCM 16-bit little-endian samples (24 kHz mono per the
            // server's output_format).
